...
1. Invoke `autonomous:keepalive` via the Skill tool. Pass `.autonomous/<NAME>.md` as the loop-file path (derive `<NAME>` from the Dispatch the same way step 5 does: ALL-CAPS, hyphens, no spaces, goal not mechanism; no tool call needed for this, it comes out of reading the Dispatch text). Keepalive returns one of three values for the loop file's `cron_job_id`: a cron job id (interactive session, cron heartbeat scheduled), `none (self-check heartbeat)` (persistent process that can self-pace, wake-up heartbeat scheduled), or `none (persistent process)` (pure batch run, no heartbeat); hold that value in-session to write into the loop file in step 5. How the probe decides is keepalive's business; see `autonomous:keepalive`. The probe goes first so that, in an interactive session, the heartbeat is the safety net during setup's generation-horizon hazards (the big template write in step 5 is the worst), not a consequence of surviving them. The loop file does not have to exist yet; a scheduled cron's Read is a no-op when the file is missing, and the next tick picks up once the file lands. If the prelaunch question in step 2 ends the mission (operator refuses the no-git-repo proposal) and a heartbeat was scheduled, tear it down before stopping so it does not outlive the abort: when the returned value is a live cron id, invoke `autonomous:cron` with `CronDelete` on it; when it is `none (self-check heartbeat)`, there is nothing to cancel (the one already-scheduled wake-up fires once, finds no loop file, and stops on its own); when it is `none (persistent process)`, nothing was scheduled.
